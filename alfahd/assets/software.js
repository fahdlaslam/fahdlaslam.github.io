				...
				$J( m_DefaultInput ).on(
					...
					function()
					{
						...
						if ( m_rgFile )
						{
							...
							{
								$m_CustomFileInput.find( '.input_file_selected_msg' ).html( m_rgFile[ 0 ][ 'name' ] );
							}
							else
							{
							...
							$J.ajax(
								...
								function ( m_Data )
								{
									if ( m_Data.bSuccess == true )
									{
										...
										$J( m_Form ).find( 'input:visible, textarea' ).val('').change();
										grecaptcha.reset();
									}
									else
									{
										var $m_ErrorModal = $J( m_Form ).find( '.modals .error_modal' );
										if ( m_Data.rgErrors !== undefined && m_Data.rgErrors.length > 0 ) {
