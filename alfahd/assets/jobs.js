			...
			function ( m_Event )
			{
				...
				if ( m_strFilter )
				{
					...
					m_Delay(
						...
						{
							$J.get(
								'https://www.valvesoftware.com/en/jobs/job-search?search=' + m_strFilter,
								function ( m_Data )
								{
									...
									if ( m_Data.length > 0 )
									{
										$J.each(
											...
											{
												m_iCounter++;
												m_strResultsHtml += '<div class="job_opening col_4"><a href="https://www.valvesoftware.com/en/jobs?job_id=' + m_rgJob[ 'reqid' ] + '"><h5 class="job_title">' + m_rgJob[ 'name' ] + '</h5></a></div>';
												if ( m_iCounter % 3 === 0 )
												{
									...
									else
									{
										...
										m_strResultsHtml += "<div class='col_12 job_opening no_results'><h5>" + m_strNoSearchResult + "<br><a href='https://www.valvesoftware.com/en/jobs'>Did we miss something?</a></h5>"
									}
	...
	function _JobCategoryExpander()
	{
		...
		$J( document ).on(
			...
			function ( m_Event )
			{
				...
				_updateUrlParameter( strJobCatParam, $m_Job.data( strJobCatParam ) )
		...
		$J( document ).on(
			...
			function ( m_Event )
			{
				m_Event.preventDefault();
				var $m_Job = $J(this).parent();
				_updateUrlParameter( strJobCatParam, '' )
				$m_Job.removeClass( 'selected' ).css( 'padding-bottom', 0 );
				$m_Job.find( '.job_openings_wrapper' ).height( 0 );
			}
		);

		var strOpenJobCategory = _getQueryParamValue( strJobCatParam );
		if ( strOpenJobCategory )
		{
			$J( '[data-' + strJobCatParam + '=' + strOpenJobCategory + '] > a' ).click();
			var iJobCatTop = $J( '[data-' + strJobCatParam + '=' + strOpenJobCategory + ']' ).offset().top - 100;
			if ( $J( window ).scrollTop() === 0 )
			{
				$J( 'html,body' ).animate( { scrollTop: iJobCatTop }, 500 );
			}
		}
	}
